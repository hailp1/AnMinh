# Auto Fix DNS Error 1000 - Simplified version
# This script will guide you through the fix process

Write-Host "=== Auto Fix DNS Error 1000 ===" -ForegroundColor Cyan
Write-Host ""

# Check if Cloudflare API credentials are in environment or .env
$email = $env:CLOUDFLARE_EMAIL
$apiKey = $env:CLOUDFLARE_API_KEY

if (-not $email -or -not $apiKey) {
    Write-Host "⚠ Cloudflare API credentials not found in environment" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Please provide your Cloudflare credentials:" -ForegroundColor Cyan
    Write-Host ""
    ...
    if (-not $apiKey) {
        ...
        Write-Host "3. Use 'Edit zone DNS' template or create custom token with:" -ForegroundColor Gray
        Write-Host "   - Account: Cloudflare Tunnel: Edit" -ForegroundColor Gray
        Write-Host "   - Zone: DNS: Edit" -ForegroundColor Gray
        Write-Host ""
        $apiKey = Read-Host "Enter your Cloudflare API Key" -AsSecureString
...
try {
    ...
    if ($zoneResponse.success -and $zoneResponse.result.Count -gt 0) {
        ...
        Write-Host "  Please check your API credentials" -ForegroundColor Yellow
        exit 1
    }
} catch {
